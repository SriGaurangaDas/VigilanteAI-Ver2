...
import OsintReport from './OsintReport'; // Import the new component
...
interface OsintData {
  // Define the structure of your OSINT data here
  whois?: any;
  ssl?: any;
}
...
export default function Home() {
  ...
  const [osintData, setOsintData] = useState<OsintData | null>(null); // New state for OSINT data
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      const [passiveRes, osintRes] = await Promise.all([
        fetch('/api/passive-scan', {
          method: 'POST',
      ...
      // Handle passive scan results
      if (!passiveRes.ok) {
        const errorData = await passiveRes.json();
        throw new Error(`Passive Scan Error: ${errorData.error || 'Something went wrong'}`);
      }
      const passiveData = await passiveRes.json();
      setFindings(passiveData);

      // Handle OSINT scan results
      if (!osintRes.ok) {
        const errorData = await osintRes.json();
        // We can choose to show this as a non-critical error
        console.error("OSINT Scan Error:", errorData.error);
      } else {
        const osintData = await osintRes.json();
        setOsintData(osintData);
      }
    ...
    <div className="bg-gray-900 text-white min-h-screen p-8">
      ...
      <main className="max-w-4xl mx-auto">
        ...
        <div className="space-y-6">
          {osintData && <OsintReport data={osintData} />}
